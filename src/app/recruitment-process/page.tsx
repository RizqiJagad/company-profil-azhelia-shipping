...
export default function RecruitmentProcessPage() {
  const getStepIcon = (name: string) => {
    switch (name) {
      ...
      case 'Search':
        return <Search className="w-7 h-7 text-white" />;
      case 'Users':
    ...
    <div className="pt-24 pb-20">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        <div className="relative">
          ...
          <div className="hidden sm:block absolute left-8 top-8 bottom-8 w-1 bg-gradient-to-b from-[#1E3A8A] via-[#3B82F6] to-[#0A2540]" />
          ...
          <div className="space-y-10">
            {recruitmentStepsData.map((item, index) => (
              ...
              >
                {/* Step Circle Badge */}
                <div className="sm:absolute sm:left-0 w-16 h-16 bg-gradient-to-br from-[#1E3A8A] to-[#3B82F6] rounded-2xl flex items-center justify-center shadow-xl text-white mb-4 sm:mb-0 flex-shrink-0">
                  {getStepIcon(item.iconName)}
                </div>

                {/* Content Card */}
                <Card className="w-full border-l-4 border-[#1E3A8A]">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xs font-bold text-[#1E3A8A] bg-blue-50 px-3 py-1 rounded-full uppercase tracking-wider">
                      Step {item.step}
                    </span>
